...
function geBgImage(variant: BGVariantType, fill: string, size: number, dotSize: number = 1) {
	switch (variant) {
		case 'dots':
			return `radial-gradient(${fill} ${dotSize}px, transparent ${dotSize}px)`;
		case 'grid':
...
const BGPattern = ({
	...
	const backgroundImage = geBgImage(variant, fill, size, dotSize);
