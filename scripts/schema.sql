...
create table if not exists content_chunks (
  ...
  source      text not null,            -- e.g. "project:hisaab-pro", "bio", "experience:0"
  title       text not null default '',  -- human-readable label for citations
  content     text not null,
  embedding   vector(3072),              -- gemini-embedding-2 (default 3072d)
...
create unique index if not exists idx_content_chunks_source
  on content_chunks (source);
